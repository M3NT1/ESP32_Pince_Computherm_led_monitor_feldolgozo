...
@app.route('/api/mqtt/rediscover', methods=['POST'])
def mqtt_rediscover():
    """MQTT discovery újraküldése a jelenlegi zónákra (force refresh)"""
    if not mqtt_client or not mqtt_client.is_connected():
        return jsonify({'success': False, 'error': 'MQTT nem csatlakozva'}), 503
    
    try:
        # Discovery újraküldése
        publish_homeassistant_discovery()
        
        return jsonify({
            'success': True,
            'message': f'{len(led_zones)} zóna discovery újraküldve. Várj ~30 másodpercet hogy a Home Assistant észrevegye.',
